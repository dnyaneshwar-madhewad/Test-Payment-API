...
function validateRequestBody(requestBodyJson) 
{
    ...
    for (const field of bodyFields) {
        ...
        if (fieldValue == null || fieldValue === "") {
            sendErrorResponseCode(Header, "ER12", `${field} must not be null or empty.`);
            return null;
        }

        if (fieldValue.length > 20) {
            sendErrorResponseCode(Header, "ER12", `${field} must be no longer than 20 characters.`);
            return null;
        }

        if (field === "Ben_Email" && !emailRegex.test(fieldValue)) {
            sendErrorResponseCode(Header, "ER12", "Invalid email format for Ben_Email.");
            return null;
        }

        if (field !== "Ben_Email" && !nonSpecialCharsRegex.test(fieldValue)) {
            sendErrorResponseCode(Header, "ER12", `${field} must be alphanumeric and no longer than 20 characters (special characters not allowed).`);
            return null;
        }
    }
