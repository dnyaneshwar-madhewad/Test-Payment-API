...
function validateRequestBody(requestBodyJson) 
{
	...
	const nonSpecialCharsRegex = /^[A-Za-z0-9\s]*$/; // Regex to match alphanumeric and spaces (no special characters)
    ...
    for (const field of requiredHeaderFieldsTra)
	{
        if (!Header?.[field]) 
		{
            sendErrorResponse(401, "Unauthorized", `Authentication Failure:Missing or invalid field: ${field}`);
            return null;
        }
	    
		if (!alphanumericRegex.test(Header[field]))
		{
			sendErrorResponse(401, "Unauthorized", `Authentication Failure:Missing or invalid field: ${field}`);			
            return null;
        }
	    
		if (Header[field].length > 16)
		{
	...
	{
        if (!Header?.[field]) {
			sendErrorResponseCode(Header, "ER002", `Missing or invalid field: ${field}`);
            return null;
        }
	    
		if (Header[field].length > 20 || !alphanumericRegex.test(Header[field]))
		{
			sendErrorResponseCode(Header, "ER002", `${field} must be alphanumeric and no longer than 16 characters`);			
            return null;
        }
    }
    ...
    if (isNaN(amount) || amount <= 0) {
        sendErrorResponseCode(Header, "ER025", "Amount value should be greater than or equal to 1 ");
        return null;
    }
    ...
    // Add validation for all other fields in the Body
    const bodyFields = [
        "Amount", "Debit_Acct_No", "Debit_Acct_Name", "Debit_IFSC", "Debit_Mobile", 
        "Debit_TrnParticulars", "Debit_PartTrnRmks", "Ben_IFSC", "Ben_Acct_No", 
        "Ben_Name", "Ben_Address", "Ben_BankName", "Ben_BankCd", "Ben_BranchCd", 
        "Ben_Email", "Ben_Mobile", "Ben_TrnParticulars", "Ben_PartTrnRmks", "Issue_BranchCd", 
        "Mode_of_Pay", "Remarks", "RptCode"
    ];
    ...
    for (const field of bodyFields) {
        ...
        if (fieldValue == null || fieldValue === "") {
            sendErrorResponseCode(Header, "ER002", `${field} must not be null or empty.`);
            return null;
        }

        if (fieldValue.length > 20) {
            sendErrorResponseCode(Header, "ER002", `${field} must be no longer than 20 characters.`);
            return null;
        }

        if (field === "Ben_Email" && !emailRegex.test(fieldValue)) {
            sendErrorResponseCode(Header, "ER002", "Invalid email format for Ben_Email.");
            return null;
        }

        if (field !== "Ben_Email" && !nonSpecialCharsRegex.test(fieldValue)) {
            sendErrorResponseCode(Header, "ER002", `${field} must be alphanumeric and no longer than 20 characters (special characters not allowed).`);
            return null;
        }
    }
